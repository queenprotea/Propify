...
#------------------------------------
    ...
    def create_ubicacion(self, ubicacion_data: schemas.UbicacionCreate):
        ...
        if not ubicacion_data.direccion_completa.strip():
            raise ValueError("complete location required")
    ...
    def update_ubicacion(self, ubicacion_id: int, ubicacion_update: schemas.UbicacionUpdate):
        if (
            ubicacion_update.direccion_completa is not None
            and not ubicacion_update.direccion_completa.strip()
        ):
            raise ValueError(
