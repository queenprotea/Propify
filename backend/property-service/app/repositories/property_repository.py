...
def get_disponible_inmuebles(db: Session):
    return (
        db.query(models.Inmueble)
        .filter(
            models.Inmueble.estado.in_([
                EstadoInmueble.ENRENTA.value,
                EstadoInmueble.ENVENTA.value
            ])
        )
        .all()
    )
...
def get_historial_estado_by_inmueble(db: Session, inmueble_id: int):
    return db.query(models.HistorialEstado).filter(models.HistorialEstado.id_inmueble == inmueble_id).order_by(models.HistorialEstado.fecha_inicio.desc()).all()
...
def create_historial_estado(
    ...
    try:
        ...
        current_historial = (
            ...
            .filter(
                models.HistorialEstado.id_inmueble == historialEstado.id_inmueble,
        ...
        db_historial_estado = models.HistorialEstado(
            id_inmueble=historialEstado.id_inmueble,
...
def get_current_historial_by_inmueble(
    ...
    return (
        db.query(models.HistorialEstado)
        .filter(
            models.HistorialEstado.id_inmueble == inmueble_id,
            models.HistorialEstado.fecha_fin.is_(None)
        )
        .first()
    )
...
def get_contactos_by_inmueble(db: Session, id_inmueble: int):
    return db.query(models.Contacto).filter(models.Contacto.id_inmueble == id_inmueble).order_by(models.Contacto.fecha.desc()).all()
...
def create_contacto(db: Session, contacto: schemas.ContactoCreate):
    try:    
        db_contacto = models.Contacto(
            ...
            id_inmueble = contacto.id_inmueble
